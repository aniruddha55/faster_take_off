if 1
    ...
    train_val_test_split([],[],timeBin_size)
    
    all_dep_spots = {'DA_015'};
    
    %     tof_dir =  '~/Google Drive/NASA-ATC/procast/eval_framework/data/table_of_factors';
    tof_dir =  '~/Google Drive/NASA-ATC/procast/eval_framework/data/table_of_factors_no_bin';
    if ispc
        tof_dir = 'data\table_of_factors'
    end
    
    test_dir = [tof_dir '/test'];
    if ispc
        test_dir = 'data\table_of_factors\test';
    end
    
    train_dir = [tof_dir '/train'];
    if ispc
        train_dir = 'data\table_of_factors\train\';
    end
    ...
    [all_BN_engines, all_engines_data] = train_BN(node_sizes_spot, train_dir);
    current_time = 1;
    ...
    for k = 1:length(all_dep_spots)
        tof_spot = load([tof_dir '/tableOfFactors_data_no_Bin_' all_dep_spots{k} '.mat']);
%         tof_spot = load([test_dir '/tableOfFactors_data_30sec_timeBin_' all_dep_spots{k} '_test.mat']);
        if ispc
            tof_spot = load([tof_dir '\tableOfFactors_data_30sec_timeBin_' all_dep_spots{s} '_test.mat' ]);
        end
        table_of_factors_data = tof_spot.tableOfFactors_data;
        ...
        [flight_trajectory] = test_BN(all_BN_engines, all_engines_data,...
            current_time, test_dir, timeBin_size);
...
fprintf('\n For bin = %d, Error metrics: (spot, merge, runway) = %f, %f, %f \n', ...
    timeBin_size, total_error_spot, total_error_merge, total_error_runway);
