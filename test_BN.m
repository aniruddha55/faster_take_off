function [flight_trajectory] = test_BN(all_BN_engines,all_engines_data,...
    current_time,table_of_factors_test_path)

files = dir(adjust_path_by_OS([table_of_factors_test_path '/*.mat']));
% iterate through each spot in the test directory
for table_of_factors_file = files'
    load([table_of_factors_test_path '/' table_of_factors_file.name]);
